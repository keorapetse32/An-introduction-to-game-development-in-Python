...
class SpaceShip(pygame.sprite.Sprite):
	def __init__(self,path,x_pos,y_pos):
		...
		self.shield_surface = pygame.image.load('image/shield.png')
		self.health = 5
...
spaceship = SpaceShip('image/spaceship.png',640,500)
...
while True: # Game loop
	for event in pygame.event.get(): # Check for events / Player input
		...
		if event.type == pygame.MOUSEBUTTONDOWN:
			new_laser = Laser('image/Laser.png',event.pos,15)
	...
	if pygame.sprite.spritecollide(spaceship_group.sprite,meteor_group,True):
		spaceship_group.sprite.get_damage(3)
